<?php
// File: user/hasil.php
// UPDATE: Support multiple elections dengan parameter ID
...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    // Jika ada parameter ID, tampilkan pemilu tersebut
    ...
    // Jika tidak ada parameter, tampilkan pemilu aktif
    $election = get_active_election();
    
    if (!$election) {
        set_flash_message('info', 'Tidak ada pemilu yang sedang berlangsung. Silakan pilih dari arsip.');
        redirect('arsip.php');
    }
...
$sudah_voting = has_voted_in_election($_SESSION['user_id'], $id_election);

// Ambil flash message
$flash = get_flash_message();

// Query untuk hasil pemilu
$query = "SELECT k.id_kandidat, k.nomor_urut, k.nama_kandidat, k.foto, k.visi, k.misi,
...
if ($sudah_voting) {
    $query_pilihan = "SELECT k.nama_kandidat, k.nomor_urut, v.waktu_voting
                      FROM voting v
    ...
    mysqli_stmt_bind_param($stmt_pilihan, "ii", $_SESSION['user_id'], $id_election);
    mysqli_stmt_execute($stmt_pilihan);
    $result_pilihan = mysqli_stmt_get_result($stmt_pilihan);
    $kandidat_dipilih = mysqli_fetch_assoc($result_pilihan);
}
    ...
    <div class="container">
        ...
        <?php if ($sudah_voting && $kandidat_dipilih): ?>
            <div class="content-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                <h3 style="margin-bottom: 15px;">✅ Pilihan Anda:</h3>
                <h2 style="font-size: 2em; margin: 10px 0;">
                ...
                <p style="opacity: 0.9; margin-top: 10px;">
                    Waktu Voting: <?php echo date('d/m/Y H:i:s', strtotime($kandidat_dipilih['waktu_voting'])); ?>
                </p>
            </div>
        <?php endif; ?>
        ...
        <div class="content-card">
            ...
            <div style="text-align: center; margin: 20px 0;">
                <h3 style="color: #667eea; font-size: 2em;"><?php echo $total_suara; ?></h3>
                <p style="color: #666;">Total Suara Masuk</p>
            </div>
        </div>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <div style="flex: 1; background-color: #e0e0e0; height: 25px; border-radius: 12px; overflow: hidden;">
                                                <div style="width: <?php echo $persentase; ?>%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); height: 100%; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 0.9em;">
        ...
        <?php if (isset($_GET['id'])): ?>
            <div style="text-align: center; margin-top: 20px;">
                <a href="arsip.php" class="btn btn-secondary">
                    ← Kembali ke Arsip
                </a>
            </div>
        <?php endif; ?>
